...
static void fetch(const std::string& ticker, const std::string& start_date, const std::string& end_date, const std::string& filename) {
    std::string command = "python3 datasets/fetch.py " + ticker + " " + start_date + " " + end_date + " datasets/" + filename;
...
static void clean() {
    ...
    std::cout << "All .csv files in the datasets folder have been deleted." << std::endl;
}
